...
from .forms import TeamForm, KnockoutResultForm
...
def get_tournament():
    t, _ = Tournament.objects.get_or_create(
        pk=1,
        defaults={'name': 'Goma Efootball Championship'}
    )
    return t
...
def _set_countdown(m, now):
    """Calcule le compte à rebours sur un match"""
    if m.played:
        m.hours_left = 0
...
def _recalculate_team_stats(tournament):
    """Recalcule points/buts de toutes les équipes depuis zéro"""
    ...
    for m in Match.objects.filter(
        ...
        home = Team.objects.get(pk=m.home_id)
        ...
        away = Team.objects.get(pk=m.away_id)
...
def _get_group_top2(tournament, group):
    """Retourne le top 2 d'un groupe basé sur les matchs joués"""
    teams = Team.objects.filter(group=group, tournament=tournament)
    ...
    for m in Match.objects.filter(
        ...
        if m.home_id not in stats or m.away_id not in stats:
            continue
    ...
    return [s['team'] for s in sorted_teams]

def _update_bracket_from_groups(tournament):
    """
    Bracket intelligent : met à jour les R16 automatiquement
    quand les groupes avancent.
    """
    r16_matches = list(
    ...
    groups_data = {}
    for g in Group.objects.filter(tournament=tournament).order_by('name'):
        top2 = _get_group_top2(tournament, g)
        if len(top2) >= 2:
            groups_data[g.name] = top2

    for i, (g1_name, pos1, g2_name, pos2) in enumerate(crossings):
        if i >= len(r16_matches):
            break

        r16_match = r16_matches[i]
        ...
        if g1_name in groups_data:
            team_home = groups_data[g1_name][pos1]
            if r16_match.home != team_home:
                r16_match.home = team_home
                changed = True

        if g2_name in groups_data:
            team_away = groups_data[g2_name][pos2]
            if r16_match.away != team_away:
                r16_match.away = team_away
                changed = True

        if changed:
            r16_match.save()

def _advance_knockout(m):
    ...
    if m.phase == 'R16' and m in r16:
        idx = r16.index(m)
        if qf and idx // 2 < len(qf):
            target = qf[idx // 2]
            if idx % 2 == 0:
                target.home = winner
            else:
    ...
    elif m.phase == 'QF' and m in qf:
        idx = qf.index(m)
        if sf and idx // 2 < len(sf):
            target = sf[idx // 2]
            if idx % 2 == 0:
                target.home = winner
            else:
...
def teams(request):
    ...
    qs = Team.objects.filter(
        tournament=t, is_validated=True
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Team.objects.filter(
                tournament=t, name__iexact=team.name
            ).exists():
            ...
            if User.objects.filter(username=username).exists():
                ...
                if Team.objects.filter(
                    tournament=t, owner=existing_user
                ).exists():
                    form.add_error('owner_username', "Tu es déjà inscrit")
                    return render(
                        request,
                        'competition/register.html',
                        {'form': form}
                    )
                user = existing_user
            else:
...
def standings(request):
    ...
    matches = Match.objects.filter(
        tournament=t, phase='group', played=True
    ).select_related('home', 'away', 'group')

    classement = {}
    for g in groups:
        teams_qs = Team.objects.filter(group=g, tournament=t)
        table = {
            team: {
                'pts': 0, 'j': 0, 'g': 0,
                'n': 0, 'p': 0, 'bp': 0, 'bc': 0
...
def team_detail(request, pk):
    ...
    palmares = {
        'titres': 0,
        'finales': Match.objects.filter(
            tournament=t, phase='F', played=True
        ).filter(Q(home=team) | Q(away=team)).count(),
        'participations': 1,
    }
...
def registration_success(request, pk):
    ...
    return render(
        request,
        'competition/registration_success.html',
        {'team': team}
    )
...
def groups_view(request):
    ...
    groups = Group.objects.filter(
        tournament=t
    ).prefetch_related('team_set').order_by('name')
...
def edit_team(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Team.objects.filter(
                tournament=t, name__iexact=new_name
...
def change_password(request):
    ...
    return render(
        request,
        'competition/change_password.html',
        {'form': form}
    )
...
def enter_result(request, match_id):
    ...
    if request.method == 'POST':
        if is_knockout:
            ...
            _recalculate_team_stats(m.tournament)
            _update_bracket_from_groups(m.tournament)
            return redirect('schedule')
...
def cancel_result(request, match_id):
    """Annule le résultat d'un match — remet à non joué"""
    m = get_object_or_404(Match, id=match_id)
    ...
    if request.method == 'POST':
        ...
        m.home_goals       = None
        m.away_goals       = None
        m.home_goals_et    = None
        m.away_goals_et    = None
        m.home_penalties   = None
        m.away_penalties   = None
        m.extra_time       = False
        ...
        if phase == 'group':
            ...
            messages.success(
                request,
                "✅ Résultat annulé — classement recalculé"
            )
            return redirect('schedule')
        else:
    ...
    return render(
        request,
        'competition/cancel_result.html',
        {'m': m}
    )
...
def manager_dashboard(request):
    ...
    validated_teams = all_teams.filter(
        is_validated=True
    ).order_by('-collective_strength', 'name')
    ...
    # ✅ CORRECT
    groups_count = Team.objects.filter(
        tournament=t,
        group__isnull=False
    ).values('group').distinct().count()

    groups_filled = f"{groups_count}/8"
...
def generate_draw(request):
    ...
    if len(teams) != 32:
        messages.error(
            request,
            f"Il faut 32 équipes validées (actuellement {len(teams)})"
        )
        return redirect('manager')
...
def generate_schedule(request):
    ...
    for group in Group.objects.filter(tournament=t):
        teams_list = list(group.team_set.all())
        if len(teams_list) != 4:
            continue
...
def generate_knockout(request):
    ...
    if len(groups) != 8:
        messages.error(
            request,
            f"⚠️ Il faut 8 groupes. Seulement {len(groups)} valide(s)."
        )
        return redirect('manager')
    ...
    for _ in range(4):
        Match.objects.create(
            tournament=t, phase='QF',
            home=None, away=None, played=False
        )
    for _ in range(2):
        Match.objects.create(
            tournament=t, phase='SF',
            home=None, away=None, played=False
        )
    Match.objects.create(
        tournament=t, phase='3P',
        home=None, away=None, played=False
    )
    Match.objects.create(
        tournament=t, phase='F',
        home=None, away=None, played=False
    )

    messages.success(
        request,
        "✅ Phase finale générée avec le vrai classement !"
    )
    return redirect('bracket')
...
def manager_edit_team(request, pk):
    ...
    return render(
        request,
        'competition/manager/edit_team.html',
        {'form': form, 'team': team}
    )
...
def manager_users(request):
    ...
    return render(
        request,
        'competition/manager/users.html',
        {'users': users}
    )
...
def set_strength(request, pk):
    ...
    if request.method == 'POST':
        ...
        team.collective_strength = (
            int(strength) if strength.isdigit() else 0
        )
        team.save()
    ...
    return render(
        request,
        'competition/manager/set_strength.html',
        {'team': team}
    )
...
def match_update(request, pk):
    """Alias vers enter_result pour compatibilité"""
    return enter_result(request, pk)
